...
erpnepal.landed_cost_taxes_and_charges.setup_triggers("Subcontracting Order");
...
frappe.ui.form.on("Subcontracting Order", {
	...
	set_queries: (frm) => {
		frm.set_query("contact_person", erpnepal.queries.contact_query);
		frm.set_query("supplier_address", erpnepal.queries.address_query);

		frm.set_query("billing_address", erpnepal.queries.company_address_query);

		frm.set_query("shipping_address", () => {
			return erpnepal.queries.company_address_query(frm.doc);
		});
	},
	...
	update_subcontracting_order_status(frm, status) {
		frappe.call({
			method: "erpnepal.subcontracting.doctype.subcontracting_order.subcontracting_order.update_subcontracting_order_status",
			args: {
				sco: frm.doc.name,
				status: status,
			},
...
erpnepal.buying.SubcontractingOrderController = class SubcontractingOrderController {
	setup() {
	...
	make_subcontracting_receipt(items) {
		frappe.model.open_mapped_doc({
			method: "erpnepal.subcontracting.doctype.subcontracting_order.subcontracting_order.make_subcontracting_receipt",
			frm: cur_frm,
			args: { items: items || [] },
			freeze: true,
...
extend_cscript(cur_frm.cscript, new erpnepal.buying.SubcontractingOrderController({ frm: cur_frm }));
