...
def update_regional_tax_settings(country, company):
	path = frappe.get_app_path("erpnepal", "regional", frappe.scrub(country))
	if os.path.exists(path.encode("utf-8")):
		try:
			module_name = f"erpnepal.regional.{frappe.scrub(country)}.setup.update_regional_tax_settings"
