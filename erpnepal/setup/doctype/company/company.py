...
from erpnepal.accounts.doctype.financial_report_template.financial_report_template import (
	sync_financial_report_templates,
)
from erpnepal.setup.setup_wizard.operations.taxes_setup import setup_taxes_and_charges
from erpnepal.stock.utils import check_pending_reposting
...
class Company(NestedSet):
	...
	def create_default_accounts(self):
		from erpnepal.accounts.doctype.account.chart_of_accounts.chart_of_accounts import create_charts
...
def install_country_fixtures(company, country):
	try:
		module_name = f"erpnepal.regional.{frappe.scrub(country)}.setup.setup"
...
def update_company_current_month_sales(company):
	...
	# Fieldname in standard ERPNepal is `total_monthly_sales`
	frappe.db.set_value(
		"Company",
		company,
		"total_monthly_sales",
...
def create_transaction_deletion_request(company):
	...
	from erpnepal.setup.doctype.transaction_deletion_record.transaction_deletion_record import (
