...
import erpnepal
from erpnepal.accounts.deferred_revenue import get_deferred_booking_accounts
...
from erpnepal.accounts.doctype.tax_withholding_entry.tax_withholding_entry import JournalTaxWithholding
...
from erpnepal.setup.utils import get_exchange_rate as _get_exchange_rate
...
class JournalEntry(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from erpnepal.accounts.doctype.journal_entry_account.journal_entry_account import JournalEntryAccount
	...
	def validate_company_for_periodic_accounting(self):
		if erpnepal.is_perpetual_inventory_enabled(self.company):
			frappe.throw(
				_(
	...
	def make_gl_entries(self, cancel=0, adv_adj=0):
		from erpnepal.accounts.general_ledger import make_gl_entries
				...
				if not blank_row:
					blank_row = self.append(
						"accounts",
						{
							...
							"cost_center": erpnepal.get_default_cost_center(self.company),
						},
					)
...
def get_outstanding(args):
	...
	company_currency = erpnepal.get_company_currency(args.get("company"))
	due_date = None
...
def get_account_details_and_party_type(account, date, company, debit=None, credit=None, exchange_rate=None):
	...
	company_currency = erpnepal.get_company_currency(company)
	account_details = frappe.get_cached_value(
