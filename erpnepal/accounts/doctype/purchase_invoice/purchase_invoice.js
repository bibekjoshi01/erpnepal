...
erpnepal.accounts.payment_triggers.setup("Purchase Invoice");
erpnepal.accounts.taxes.setup_tax_filters("Purchase Taxes and Charges");
erpnepal.accounts.taxes.setup_tax_validations("Purchase Invoice");
erpnepal.buying.setup_buying_controller();

erpnepal.accounts.PurchaseInvoice = class PurchaseInvoice extends erpnepal.buying.BuyingController {
	setup(doc) {
	...
	unblock_invoice() {
		const me = this;
		frappe.call({
			method: "erpnepal.accounts.doctype.purchase_invoice.purchase_invoice.unblock_invoice",
	...
	make_comment_dialog_and_block_invoice() {
		...
		this.dialog.set_primary_action(__("Save"), function () {
			...
			frappe.call({
				method: "erpnepal.accounts.doctype.purchase_invoice.purchase_invoice.block_invoice",
				args: {
	...
	set_release_date(data) {
		return frappe.call({
			method: "erpnepal.accounts.doctype.purchase_invoice.purchase_invoice.change_release_date",
			args: data,
	...
	make_inter_company_invoice(frm) {
		frappe.model.open_mapped_doc({
			method: "erpnepal.accounts.doctype.purchase_invoice.purchase_invoice.make_inter_company_sales_invoice",
			frm: frm,
		});
	}
	...
	make_debit_note() {
		frappe.model.open_mapped_doc({
			method: "erpnepal.accounts.doctype.purchase_invoice.purchase_invoice.make_debit_note",
			frm: this.frm,
		});
	}
};

cur_frm.script_manager.make(erpnepal.accounts.PurchaseInvoice);
...
frappe.ui.form.on("Purchase Invoice", {
	...
	mode_of_payment: function (frm) {
		erpnepal.accounts.pos.get_payment_mode_account(frm, frm.doc.mode_of_payment, function (account) {
