...
frappe.treeview_settings["Account"] = {
	...
	filters: [
		{
			fieldname: "company",
			fieldtype: "Select",
			options: erpnepal.utils.get_tree_options("company"),
			label: __("Company"),
			render_on_toolbar: true,
			default: erpnepal.utils.get_tree_default("company"),
			on_change: function () {
				...
				frappe.call({
					method: "erpnepal.accounts.doctype.account.account.get_root_company",
					args: {
						company: company,
					},
					callback: function (r) {
	...
	root_label: "Accounts",
	get_tree_nodes: "erpnepal.accounts.utils.get_children",
		...
		frappe.db.get_single_value("Accounts Settings", "show_balance_in_coa").then((value) => {
			if (value) {
				const get_balances = frappe.call({
					method: "erpnepal.accounts.utils.get_account_balances",
					args: {
						accounts: accounts,
	...
	add_tree_node: "erpnepal.accounts.utils.add_ac",
	menu_items: [
		{
		...
		},
		{
			...
			click: function (node, btn) {
				frappe.route_options = {
					from_date: erpnepal.utils.get_fiscal_year(frappe.datetime.get_today(), true)[1],
					to_date: erpnepal.utils.get_fiscal_year(frappe.datetime.get_today(), true)[2],
					company:
