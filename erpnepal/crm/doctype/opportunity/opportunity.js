...
frappe.provide("erpnepal.crm");
erpnepal.pre_sales.set_as_lost("Opportunity");
...
frappe.ui.form.on("Opportunity", {
	...
	party_name: function (frm) {
		...
		if (frm.doc.opportunity_from == "Customer") {
			erpnepal.utils.get_party_details(frm);
		} else if (frm.doc.opportunity_from == "Lead") {
			erpnepal.utils.map_current_doc({
				method: "erpnepal.crm.doctype.lead.lead.make_opportunity",
	...
	customer_address: function (frm, cdt, cdn) {
		erpnepal.utils.get_address_display(frm, "customer_address", "address_display", false);
	},

	contact_person: erpnepal.utils.get_contact_details,
	...
	refresh: function (frm) {
		...
		if (!frm.is_new() && doc.status !== "Lost") {
			...
			let company_currency = erpnepal.get_currency(frm.doc.company);
	...
	make_supplier_quotation: function (frm) {
		frappe.model.open_mapped_doc({
			method: "erpnepal.crm.doctype.opportunity.opportunity.make_supplier_quotation",
			frm: frm,
		});
	},
	...
	make_request_for_quotation: function (frm) {
		frappe.model.open_mapped_doc({
			method: "erpnepal.crm.doctype.opportunity.opportunity.make_request_for_quotation",
			frm: frm,
		});
	},
...
erpnepal.crm.Opportunity = class Opportunity extends frappe.ui.form.Controller {
	onload() {
	...
	setup_queries() {
		...
		if (me.frm.doc.opportunity_from == "Lead") {
			me.frm.set_query("party_name", erpnepal.queries["lead"]);
		} else if (me.frm.doc.opportunity_from == "Customer") {
			me.frm.set_query("party_name", erpnepal.queries["customer"]);
	...
	create_quotation() {
		frappe.model.open_mapped_doc({
			method: "erpnepal.crm.doctype.opportunity.opportunity.make_quotation",
			frm: this.frm,
		});
	}
	...
	make_customer() {
		frappe.model.open_mapped_doc({
			method: "erpnepal.crm.doctype.opportunity.opportunity.make_customer",
			frm: this.frm,
		});
	}

	show_notes() {
		const crm_notes = new erpnepal.utils.CRMNotes({
			frm: this.frm,
	...
	show_activities() {
		const crm_activities = new erpnepal.utils.CRMActivities({
			frm: this.frm,
...
extend_cscript(cur_frm.cscript, new erpnepal.crm.Opportunity({ frm: cur_frm }));

cur_frm.cscript.item_code = function (doc, cdt, cdn) {
	var d = locals[cdt][cdn];
	if (d.item_code) {
		return frappe.call({
			method: "erpnepal.crm.doctype.opportunity.opportunity.get_item_details",
