...
erpnepal.PointOfSale.ItemSelector = class {
	...
	async get_parent_item_group() {
		const r = await frappe.call({
			method: "erpnepal.selling.page.point_of_sale.point_of_sale.get_parent_item_group",
			args: {
	...
	get_items({ start = 0, page_length = 40, search_term = "" }) {
		...
		return frappe.call({
			method: "erpnepal.selling.page.point_of_sale.point_of_sale.get_items",
			freeze: true,
	...
	make_search_bar() {
		...
		this.item_group_field = frappe.ui.form.make_control({
			df: {
				...
				get_query: function () {
					return {
						query: "erpnepal.selling.page.point_of_sale.point_of_sale.item_group_query",
						filters: {
