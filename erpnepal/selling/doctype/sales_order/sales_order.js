...
erpnepal.accounts.taxes.setup_tax_validations("Sales Order");
...
frappe.ui.form.on("Sales Order", {
	...
	refresh: function (frm) {
		...
		if (frm.doc.docstatus === 1) {
			if (
				...
				frm.add_custom_button(__("Update Items"), () => {
					erpnepal.utils.update_child_items({
						frm: frm,
						child_docname: "items",
		...
		if (frm.doc.docstatus === 0) {
			...
			if (frm.doc.docstatus === 0 && !frm.doc.is_subcontracted) {
				frappe.call({
					method: "erpnepal.selling.doctype.sales_order.sales_order.get_stock_reservation_status",
					callback: function (r) {
						if (!r.message) {
		...
		frm.add_custom_button(
			...
			() => {
				erpnepal.utils.map_current_doc({
					method: "erpnepal.buying.doctype.purchase_order.purchase_order.make_inter_company_sales_order",
	...
	onload: function (frm) {
		...
		erpnepal.queries.setup_queries(frm, "Warehouse", function () {
			return {
				filters: [
		...
		frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
			...
			if (row.item_code) {
				query.query = "erpnepal.controllers.queries.warehouse_query";
		...
		const dialog = new frappe.ui.Dialog({
			...
			fields: [
				...
				{ fieldtype: "Column Break" },
				{
					...
					get_query: () => {
						return {
							query: "erpnepal.controllers.queries.get_filtered_child_rows",
							filters: {
	...
	cancel_stock_reservation_entries(frm) {
		...
		frappe
			.call({
				method: "erpnepal.stock.doctype.stock_reservation_entry.stock_reservation_entry.get_stock_reservation_entries_for_voucher",
				args: {
	...
	get_subcontracting_boms_for_finished_goods: function (fg_item) {
		return frappe.call({
			method: "erpnepal.subcontracting.doctype.subcontracting_bom.subcontracting_bom.get_subcontracting_boms_for_finished_goods",
			args: {
				fg_items: fg_item,
			},
		});
	...
	get_subcontracting_boms_for_service_item: function (service_item) {
		return frappe.call({
			method: "erpnepal.subcontracting.doctype.subcontracting_bom.subcontracting_bom.get_subcontracting_boms_for_service_item",
			args: {
...
frappe.ui.form.on("Sales Order Item", {
	...
	delivery_date: function (frm, cdt, cdn) {
		if (!frm.doc.delivery_date) {
			erpnepal.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "delivery_date");
		}
	},
...
erpnepal.selling.SalesOrderController = class SalesOrderController extends erpnepal.selling.SellingController {
	setup(doc) {
	...
	refresh(doc, dt, dn) {
		...
		if (
			...
			this.frm.add_custom_button(
				__("Quotation"),
				function () {
					let d = erpnepal.utils.map_current_doc({
						method: "erpnepal.selling.doctype.quotation.quotation.make_sales_order",
	...
	create_pick_list() {
		frappe.model.open_mapped_doc({
			method: "erpnepal.selling.doctype.sales_order.sales_order.create_pick_list",
			frm: this.frm,
		});
	}
	...
	make_work_order() {
		var me = this;
		me.frm.call({
			method: "erpnepal.selling.doctype.sales_order.sales_order.get_work_order_items",
			args: {
	...
	make_material_request() {
		frappe.model.open_mapped_doc({
			method: "erpnepal.selling.doctype.sales_order.sales_order.make_material_request",
			frm: this.frm,
		});
	}
	...
	make_raw_material_request_dialog(r) {
		...
		var d = new frappe.ui.Dialog({
			...
			primary_action: function () {
				...
				me.frm.call({
					method: "erpnepal.selling.doctype.sales_order.sales_order.make_raw_material_request",
					args: {
						items: data,
						company: me.frm.doc.company,
						sales_order: me.frm.docname,
	...
	make_delivery_note(delivery_dates, for_reserved_stock = false) {
		frappe.model.open_mapped_doc({
			method: "erpnepal.selling.doctype.sales_order.sales_order.make_delivery_note",
			frm: this.frm,
			args: {
				delivery_dates,
	...
	make_maintenance_schedule() {
		frappe.model.open_mapped_doc({
			method: "erpnepal.selling.doctype.sales_order.sales_order.make_maintenance_schedule",
			frm: this.frm,
		});
	}
	...
	make_project() {
		frappe.model.open_mapped_doc({
			method: "erpnepal.selling.doctype.sales_order.sales_order.make_project",
			frm: this.frm,
		});
	}
	...
	make_inter_company_order() {
		frappe.model.open_mapped_doc({
			method: "erpnepal.selling.doctype.sales_order.sales_order.make_inter_company_purchase_order",
			frm: this.frm,
		});
	}
	...
	make_maintenance_visit() {
		frappe.model.open_mapped_doc({
			method: "erpnepal.selling.doctype.sales_order.sales_order.make_maintenance_visit",
			frm: this.frm,
		});
	}
	...
	make_purchase_order() {
		...
		var dialog = new frappe.ui.Dialog({
			...
			primary_action(args) {
				...
				dialog.hide();
				return frappe.call({
					method: "erpnepal.selling.doctype.sales_order.sales_order.make_purchase_order",
	...
	update_status(label, status) {
		...
		frappe.call({
			method: "erpnepal.selling.doctype.sales_order.sales_order.update_status",
	...
	make_subcontracting_inward_order() {
		frappe.model.open_mapped_doc({
			method: "erpnepal.selling.doctype.sales_order.sales_order.make_subcontracting_inward_order",
			frm: this.frm,
...
extend_cscript(cur_frm.cscript, new erpnepal.selling.SalesOrderController({ frm: cur_frm }));
