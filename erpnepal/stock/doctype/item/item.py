...
import erpnepal
from erpnepal.controllers.item_variant import (
...
from erpnepal.stock.doctype.item_default.item_default import ItemDefault
from erpnepal.stock.utils import get_valuation_method
...
class Item(Document):
	...
	if TYPE_CHECKING:
		...
		from erpnepal.stock.doctype.item_barcode.item_barcode import ItemBarcode
		from erpnepal.stock.doctype.item_customer_detail.item_customer_detail import ItemCustomerDetail
		from erpnepal.stock.doctype.item_default.item_default import ItemDefault
		from erpnepal.stock.doctype.item_reorder.item_reorder import ItemReorder
		from erpnepal.stock.doctype.item_supplier.item_supplier import ItemSupplier
		from erpnepal.stock.doctype.item_tax.item_tax import ItemTax
		from erpnepal.stock.doctype.item_variant_attribute.item_variant_attribute import ItemVariantAttribute
		from erpnepal.stock.doctype.uom_conversion_detail.uom_conversion_detail import UOMConversionDetail
			...
			item_price = frappe.get_doc(
				{
					...
					"uom": self.stock_uom,
					"brand": self.brand,
					"currency": erpnepal.get_default_currency(),
			...
			for item_barcode in self.barcodes:
				...
				if item_barcode.barcode:
					...
					if item_barcode.barcode_type:
						barcode_type = convert_erpnepal_to_barcodenumber(
	...
	def recalculate_bin_qty(self, new_name):
		from erpnepal.stock.stock_balance import repost_stock
		...
		if self.has_variants:
			...
			if variants:
				if len(variants) <= 30:
					...
					frappe.enqueue(
						"erpnepal.stock.doctype.item.item.update_variants",
						variants=variants,
						template=self,
						now=frappe.in_test,
						timeout=600,
...
def convert_erpnepal_to_barcodenumber(erpnepal_number, barcode):
	if erpnepal_number == "EAN":
		ean_type = {
			8: "EAN8",
			13: "EAN13",
		}
	...
	if erpnepal_number in convert:
		return convert[erpnepal_number]
...
def get_asset_naming_series():
	from erpnepal.assets.doctype.asset.asset import get_asset_naming_series
