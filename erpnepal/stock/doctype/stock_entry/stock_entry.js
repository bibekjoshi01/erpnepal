...
frappe.provide("erpnepal.stock");
frappe.provide("erpnepal.accounts.dimensions");

erpnepal.landed_cost_taxes_and_charges.setup_triggers("Stock Entry");
...
frappe.ui.form.on("Stock Entry", {
	setup: function (frm) {
		...
		frm.set_query("batch_no", "items", function (doc, cdt, cdn) {
			...
			if (!item.item_code) {
				...
				return {
					query: "erpnepal.controllers.queries.get_batch_no",
					filters: filters,
				};
			}
		});
		...
		frm.set_query("project", "items", function (doc) {
			return {
				query: "erpnepal.controllers.queries.get_project_name",
				filters: {
					company: doc.company,
				},
			};
		...
		erpnepal.accounts.dimensions.setup_dimension_filters(frm, frm.doctype);
		...
		if (!check_should_not_attach_bom_items(frm.doc.bom_no)) {
			erpnepal.accounts.dimensions.update_dimension(frm, frm.doctype);
		}

		if (frm.doc.purpose == "Receive from Customer") {
			frm.set_query("against_fg", "items", function () {
				return {
					query: "erpnepal.controllers.subcontracting_inward_controller.get_fg_reference_names",
					filters: {
	...
	setup_quality_inspection: function (frm) {
		...
		if (!frm.is_new() && frm.doc.docstatus === 0) {
			frm.add_custom_button(
				...
				() => {
					let transaction_controller = new erpnepal.TransactionController({ frm: frm });
		...
		frm.set_query("quality_inspection", "items", function (doc, cdt, cdn) {
			...
			return {
				query: "erpnepal.stock.doctype.quality_inspection.quality_inspection.quality_inspection_query",
				filters: {
	...
	refresh: function (frm) {
		...
		if (frm.doc.items) {
			...
			if (frm.doc.docstatus == 0 && has_alternative) {
				frm.add_custom_button(__("Alternate Item"), () => {
					erpnepal.utils.select_alternate_items({
						frm: frm,
						child_docname: "items",
			...
			if (
				...
				frm.add_custom_button(__("End Transit"), function () {
					frappe.model.open_mapped_doc({
						method: "erpnepal.stock.doctype.stock_entry.stock_entry.make_stock_in_entry",
						frm: frm,
					});
				});
			}
		...
		if (frm.doc.docstatus === 0 && !frm.doc.subcontracting_inward_order) {
			frm.add_custom_button(
				__("Purchase Invoice"),
				function () {
					erpnepal.utils.map_current_doc({
						method: "erpnepal.accounts.doctype.purchase_invoice.purchase_invoice.make_stock_entry",
			...
			frm.add_custom_button(
				...
				function () {
					...
					const d = erpnepal.utils.map_current_doc({
						method: "erpnepal.stock.doctype.material_request.material_request.make_stock_entry",
		...
		if (
			...
			frm.add_custom_button(
				...
				function () {
					frappe.call({
						method: "erpnepal.stock.doctype.stock_entry.stock_entry.get_expired_batch_items",
						freeze: true,
						callback: function (r) {
							if (!r.exc && r.message) {
								frm.set_value("items", []);
	...
	validate_purpose_consumption: function (frm) {
		frappe
			.call({
				method: "erpnepal.manufacturing.doctype.manufacturing_settings.manufacturing_settings.is_material_consumption_enabled",
			})
			.then((r) => {
	...
	make_retention_stock_entry: function (frm) {
		frappe.call({
			method: "erpnepal.stock.doctype.stock_entry.stock_entry.move_sample_to_retention_warehouse",
			args: {
	...
	toggle_display_account_head: function (frm) {
		var enabled = erpnepal.is_perpetual_inventory_enabled(frm.doc.company);
	...
	set_basic_rate: function (frm, cdt, cdn) {
		...
		if (item.item_code || item.serial_no) {
			frappe.call({
				method: "erpnepal.stock.utils.get_incoming_rate",
				args: {
					args: args,
				},
				callback: function (r) {
	...
	get_warehouse_details: function (frm, cdt, cdn) {
		...
		if (!child.bom_no) {
			frappe.call({
				method: "erpnepal.stock.doctype.stock_entry.stock_entry.get_warehouse_details",
				args: {
					args: {
	...
	get_items_from_bom: function (frm) {
		...
		d.get_input("fetch").on("click", function () {
			...
			frappe.call({
				method: "erpnepal.manufacturing.doctype.bom.bom.get_bom_items",
				args: values,
				callback: function (r) {
					if (!r.message) {
						frappe.throw(__("BOM does not contain any stock item"));
					} else {
						erpnepal.utils.remove_empty_first_row(frm, "items");
	...
	source_warehouse_address: function (frm) {
		erpnepal.utils.get_address_display(frm, "source_warehouse_address", "source_address_display", false);
	},

	target_warehouse_address: function (frm) {
		erpnepal.utils.get_address_display(frm, "target_warehouse_address", "target_address_display", false);
	},
	...
	apply_putaway_rule: function (frm) {
		if (frm.doc.apply_putaway_rule) erpnepal.apply_putaway_rule(frm, frm.doc.purpose);
	},
	...
	purchase_order: (frm) => {
		if (frm.doc.purchase_order) {
			...
			erpnepal.utils.map_current_doc({
				method: "erpnepal.stock.doctype.stock_entry.stock_entry.get_items_from_subcontract_order",
...
frappe.ui.form.on("Stock Entry Detail", {
	...
	uom(doc, cdt, cdn) {
		...
		if (d.uom && d.item_code) {
			return frappe.call({
				method: "erpnepal.stock.doctype.stock_entry.stock_entry.get_uom_details",
				args: {
					item_code: d.item_code,
					uom: d.uom,
					qty: d.qty,
				...
				callback: function (r) {
					if (r.message) {
						...
						if (no_batch_serial_number_value && !frappe.flags.hide_serial_batch_dialog) {
							...
							erpnepal.stock.select_batch_and_serial_no(frm, d);
						} else {
	...
	expense_account(frm, cdt, cdn) {
		erpnepal.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "expense_account");
	},

	cost_center(frm, cdt, cdn) {
		erpnepal.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "cost_center");
	},
	...
	add_serial_batch_bundle(frm, cdt, cdn) {
		var child = locals[cdt][cdn];
		erpnepal.stock.select_batch_and_serial_no(frm, child);
	},
});
...
var validate_sample_quantity = function (frm, cdt, cdn) {
	...
	if (d.sample_quantity && d.transfer_qty && frm.doc.purpose == "Material Receipt") {
		frappe.call({
			method: "erpnepal.stock.doctype.stock_entry.stock_entry.validate_sample_quantity",
			args: {
				batch_no: d.batch_no,
				item_code: d.item_code,
...
erpnepal.stock.StockEntry = class StockEntry extends erpnepal.stock.StockController {
	setup() {
		var me = this;

		this.barcode_scanner = new erpnepal.utils.BarcodeScanner({
			frm: this.frm,
			warehouse_field: (doc) => {
		...
		this.frm.fields_dict.items.grid.get_field("item_code").get_query = function () {
			return erpnepal.queries.item({ is_stock_item: 1 });
		};
		...
		this.frm.set_query("uom", "items", function (doc, cdt, cdn) {
			...
			return {
				query: "erpnepal.controllers.queries.get_item_uom_query",
				filters: {
					item_code: row.item_code,
				},
			};
		...
		this.frm.fields_dict.items.grid.get_field("expense_account").get_query = function () {
			if (erpnepal.is_perpetual_inventory_enabled(me.frm.doc.company)) {
				return {
					filters: {
						company: me.frm.doc.company,
						is_group: 0,
		...
		this.frm.set_query("supplier_address", erpnepal.queries.address_query);
	...
	refresh() {
		erpnepal.toggle_naming_series();
		this.toggle_related_fields(this.frm.doc);
		this.toggle_enable_bom();
		this.show_stock_ledger();
		this.set_fields_onload_for_line_item();
		erpnepal.utils.view_serial_batch_nos(this.frm);
		if (this.frm.doc.docstatus === 1 && erpnepal.is_perpetual_inventory_enabled(this.frm.doc.company)) {
			this.show_general_ledger();
		}
		erpnepal.hide_company(this.frm);
		erpnepal.utils.add_item(this.frm);
		erpnepal.accounts.ledger_preview.show_accounting_ledger_preview(this.frm);
		erpnepal.accounts.ledger_preview.show_stock_ledger_preview(this.frm);
	}
	...
	company() {
		if (this.frm.doc.company) {
			...
			erpnepal.accounts.dimensions.update_dimension(this.frm, this.frm.doctype);
	...
	set_default_account(company_fieldname, fieldname) {
		...
		return this.frm.call({
			method: "erpnepal.accounts.utils.get_company_default",
			args: {
			...
			return this.frm.call({
				...
				callback: function (r) {
					...
					if (me.frm.doc.bom_no) {
						...
						erpnepal.accounts.dimensions.update_dimension(me.frm, me.frm.doctype);
					}
				},
			});
		}
	...
	work_order() {
		...
		return frappe.call({
			method: "erpnepal.stock.doctype.stock_entry.stock_entry.get_work_order_details",
			args: {
	...
	set_warehouse_in_children(child_table, warehouse_field, warehouse) {
		let transaction_controller = new erpnepal.TransactionController();
		transaction_controller.autofill_warehouse(child_table, warehouse_field, warehouse);
	}

	items_on_form_rendered(doc, grid_row) {
		erpnepal.setup_serial_or_batch_no();
	}
	...
	toggle_related_fields(doc) {
		...
		if (erpnepal.stock.is_subcontracting_or_return_transfer(doc)) {
			doc.customer =
				doc.customer_name =
	...
	supplier(doc) {
		erpnepal.utils.get_party_details(this.frm, null, null, null);
	}
};

erpnepal.stock.select_batch_and_serial_no = (frm, item) => {
	let path = "assets/erpnepal/js/utils/serial_no_batch_selector.js";
	...
	frappe.db.get_value("Item", item.item_code, ["has_batch_no", "has_serial_no"]).then((r) => {
		if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
			...
			new erpnepal.SerialBatchPackageSelector(frm, item, (r) => {
				if (r) {
...
erpnepal.stock.is_subcontracting_or_return_transfer = (doc) => {
...
function attach_bom_items(bom_no) {
	...
	frappe.db.get_doc("BOM", bom_no).then((bom) => {
		...
		erpnepal.stock.bom = { name, items: {} };
		items.forEach((item) => {
			erpnepal.stock.bom.items[item.item_code] = item;
		});
	});
}

function check_should_not_attach_bom_items(bom_no) {
	return bom_no === undefined || (erpnepal.stock.bom && erpnepal.stock.bom.name === bom_no);
}

extend_cscript(cur_frm.cscript, new erpnepal.stock.StockEntry({ frm: cur_frm }));
