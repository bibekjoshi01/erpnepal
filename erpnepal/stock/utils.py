...
import erpnepal
from erpnepal.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle import get_available_serial_nos
...
from erpnepal.stock.valuation import FIFOValuation, LIFOValuation
...
def get_incoming_rate(args, raise_error_if_no_rate=True):
	...
	from erpnepal.stock.stock_ledger import get_previous_sle, get_valuation_rate
	...
	if in_rate is None:
		...
		in_rate = get_valuation_rate(
			...
			currency=erpnepal.get_company_currency(args.get("company")),
	...
	if reposting_pending and throw_error:
		...
		frappe.msgprint(
			...
			primary_action={
				...
				"client_action": "erpnepal.route_to_pending_reposts",
				"args": filters,
			},
		)
...
def scan_barcode(search_value: str, ctx: dict | str | None = None) -> BarcodeScanResult:
	def set_cache(data: BarcodeScanResult):
		frappe.cache().set_value(f"erpnepal:barcode_scan:{search_value}", data, expires_in_sec=120)
		_update_item_info(data, ctx)

	def get_cache() -> BarcodeScanResult | None:
		data = frappe.cache().get_value(f"erpnepal:barcode_scan:{search_value}")
		if not data:
			return
