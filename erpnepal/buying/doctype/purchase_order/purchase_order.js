...
frappe.provide("erpnepal.buying");
frappe.provide("erpnepal.accounts.dimensions");

cur_frm.cscript.tax_table = "Purchase Taxes and Charges";

erpnepal.accounts.taxes.setup_tax_filters("Purchase Taxes and Charges");
erpnepal.accounts.taxes.setup_tax_validations("Purchase Order");
erpnepal.buying.setup_buying_controller();
...
frappe.ui.form.on("Purchase Order", {
	setup: function (frm) {
		...
		frm.set_query("expense_account", "items", function () {
			return {
				query: "erpnepal.controllers.queries.get_expense_account",
	...
	get_materials_from_supplier: function (frm) {
		...
		if (po_details && po_details.length) {
			frm.add_custom_button(
				...
				() => {
					frm.call({
						method: "erpnepal.controllers.subcontracting_controller.get_materials_from_supplier",
						freeze: true,
	...
	onload: function (frm) {
		...
		erpnepal.queries.setup_queries(frm, "Warehouse", function () {
			return erpnepal.queries.warehouse(frm.doc);
		});
...
frappe.ui.form.on("Purchase Order Item", {
	schedule_date: function (frm, cdt, cdn) {
		...
		if (row.schedule_date) {
			if (!frm.doc.schedule_date) {
				erpnepal.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "schedule_date");
			} else {
				set_schedule_date(frm);
			}
		}
...
erpnepal.buying.PurchaseOrderController = class PurchaseOrderController extends (
	erpnepal.buying.BuyingController
) {
	setup() {
	...
	get_items_from_open_material_requests() {
		erpnepal.utils.map_current_doc({
			method: "erpnepal.stock.doctype.material_request.material_request.make_purchase_order_based_on_supplier",
			args: {
			...
			get_query_method:
				"erpnepal.stock.doctype.material_request.material_request.get_material_requests_based_on_supplier",
		});
	}
	...
	make_stock_entry() {
		frappe.call({
			method: "erpnepal.controllers.subcontracting_controller.make_rm_stock_entry",
			args: {
	...
	make_purchase_receipt() {
		frappe.model.open_mapped_doc({
			method: "erpnepal.buying.doctype.purchase_order.purchase_order.make_purchase_receipt",
			frm: this.frm,
	...
	make_purchase_invoice() {
		frappe.model.open_mapped_doc({
			method: "erpnepal.buying.doctype.purchase_order.purchase_order.make_purchase_invoice",
			frm: this.frm,
		});
	}
	...
	make_subcontracting_order() {
		frappe.model.open_mapped_doc({
			method: "erpnepal.buying.doctype.purchase_order.purchase_order.make_subcontracting_order",
			frm: this.frm,
	...
	add_from_mappers() {
		...
		this.frm.add_custom_button(
			...
			function () {
				erpnepal.utils.map_current_doc({
					method: "erpnepal.stock.doctype.material_request.material_request.make_purchase_order",
					source_doctype: "Material Request",
					target: me.frm,
					setters: {
		...
		this.frm.add_custom_button(
			...
			function () {
				erpnepal.utils.map_current_doc({
					method: "erpnepal.buying.doctype.supplier_quotation.supplier_quotation.make_purchase_order",
		...
		this.frm.add_custom_button(
			...
			function () {
				...
				frappe.call({
					method: "erpnepal.buying.utils.get_linked_material_requests",
					args: {
						items: my_items,
					},
					callback: function (r) {
...
extend_cscript(cur_frm.cscript, new erpnepal.buying.PurchaseOrderController({ frm: cur_frm }));

cur_frm.cscript.update_status = function (label, status) {
	frappe.call({
		method: "erpnepal.buying.doctype.purchase_order.purchase_order.update_status",
...
function set_schedule_date(frm) {
	if (frm.doc.schedule_date) {
		erpnepal.utils.copy_value_in_all_rows(
			frm.doc,
			frm.doc.doctype,
			frm.doc.name,
			"items",
...
frappe.ui.form.on("Purchase Order", "is_subcontracted", function (frm) {
	if (frm.doc.is_old_subcontracting_flow) {
		erpnepal.buying.get_default_bom(frm);
	}
});
