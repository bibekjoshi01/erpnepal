...
erpnepal.accounts.bank_reconciliation.NumberCardManager = class NumberCardManager {
	constructor(opts) {
	...
	make_cards() {
		...
		chart_data.forEach((summary) => {
			let number_card = new erpnepal.accounts.NumberCard(summary);
...
erpnepal.accounts.NumberCard = class NumberCard {
	constructor(options) {
