...
frappe.provide("erpnepal.selling");

erpnepal.sales_common = {
	setup_selling_controller: function () {
		erpnepal.selling.SellingController = class SellingController extends erpnepal.TransactionController {
			setup() {
				super.setup();
			...
			setup_queries() {
				...
				$.each(
					...
					function (i, opts) {
						if (me.frm.fields_dict[opts[0]]) me.frm.set_query(opts[0], erpnepal.queries[opts[1]]);
					}
				);

				me.frm.set_query("contact_person", erpnepal.queries.contact_query);
				me.frm.set_query("company_contact_person", erpnepal.queries.company_contact_query);
				me.frm.set_query("customer_address", erpnepal.queries.address_query);
				me.frm.set_query("shipping_address_name", erpnepal.queries.address_query);
				me.frm.set_query("dispatch_address_name", erpnepal.queries.dispatch_address_query);
				me.frm.set_query("company_address", erpnepal.queries.company_address_query);
			...
			set_default_company_address() {
				...
				if (this.frm.doc.company) {
					frappe.call({
						method: "erpnepal.setup.doctype.company.company.get_default_company_address",
						args: {
			...
			customer_address() {
				erpnepal.utils.get_address_display(this.frm, "customer_address");
			...
			shipping_address_name() {
				erpnepal.utils.get_address_display(this.frm, "shipping_address_name", "shipping_address");
				erpnepal.utils.set_taxes_from_address(
					this.frm,
					"shipping_address_name",
					"customer_address",
			...
			dispatch_address_name() {
				erpnepal.utils.get_address_display(this.frm, "dispatch_address_name", "dispatch_address");
			}
			...
			project() {
				...
				if (["Delivery Note", "Sales Invoice", "Sales Order"].includes(this.frm.doc.doctype)) {
					if (this.frm.doc.project) {
						frappe.call({
							method: "erpnepal.projects.doctype.project.project.get_cost_center_name",
