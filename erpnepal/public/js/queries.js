...
frappe.provide("erpnepal.queries");
$.extend(erpnepal.queries, {
	user: function () {
	...
	item: function (filters) {
		var args = { query: "erpnepal.controllers.queries.item_query" };
		if (filters) args["filters"] = filters;
		return args;
	},

	bom: function () {
		return { query: "erpnepal.controllers.queries.bom" };
	},

	task: function () {
		return { query: "erpnepal.projects.utils.query_task" };
	},
	...
	employee: function () {
		return { query: "erpnepal.controllers.queries.employee_query" };
	},
	...
	get_filtered_dimensions: function (doc, child_fields, dimension, company) {
		...
		return {
			query: "erpnepal.controllers.queries.get_filtered_dimensions",
			filters: {
				dimension: dimension,
				account: account,
				company: company,
...
erpnepal.queries.setup_queries = function (frm, options, query_fn) {
	var me = this;
...
*/
erpnepal.queries.setup_warehouse_query = function (frm) {
	frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
		var row = locals[cdt][cdn];
		var filters = erpnepal.queries.warehouse(frm.doc);
		if (row.item_code) {
			$.extend(filters, { query: "erpnepal.controllers.queries.warehouse_query" });
