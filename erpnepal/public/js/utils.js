...
frappe.provide("erpnepal");
frappe.provide("erpnepal.utils");
frappe.provide("erpnepal.stock.utils");
...
$.extend(erpnepal, {
	...
	hide_company: function (frm) {
		if (frm?.fields_dict.company) {
			...
			} else if (erpnepal.last_selected_company) {
				if (!frm.doc.company) frm.set_value("company", erpnepal.last_selected_company);
			}
		}
	},
...
$.extend(erpnepal.utils, {
	set_party_dashboard_indicators: function (frm) {
		if (frm.doc.__onload && frm.doc.__onload.dashboard_info) {
			var company_wise_info = frm.doc.__onload.dashboard_info;
			if (company_wise_info.length > 1) {
				company_wise_info.forEach(function (info) {
					erpnepal.utils.add_indicator_for_multicompany(frm, info);
				});
	...
	get_terms: function (tc_name, doc, callback) {
		if (tc_name) {
			return frappe.call({
				method: "erpnepal.setup.doctype.terms_and_conditions.terms_and_conditions.get_terms_and_conditions",
				args: {
	...
	add_inventory_dimensions: function (report_name, index) {
		...
		frappe.call({
			method: "erpnepal.stock.doctype.inventory_dimension.inventory_dimension.get_inventory_dimensions",
			callback: function (r) {
	...
	get_fiscal_year: function (date, with_dates = false, raise_on_missing = true) {
		...
		if (
			...
			frappe.call({
				method: "erpnepal.accounts.utils.get_fiscal_year",
...
erpnepal.utils.select_alternate_items = function (opts) {
	const frm = opts.frm;
			...
			{ fieldtype: "Section Break", label: __("Items") },
			{
				...
				fields: [
					...
					},
					{
						...
						get_query: (e) => {
							return {
								query: "erpnepal.stock.doctype.item_alternative.item_alternative.get_alternative_items",
								filters: {
...
erpnepal.utils.update_child_items = function (opts) {
	const frm = opts.frm;
	...
	let dialog = new frappe.ui.Dialog({
		...
		update_items: function () {
			...
			frappe.call({
				method: "erpnepal.controllers.accounts_controller.update_child_qty_rate",
				freeze: true,
				args: {
...
erpnepal.utils.map_current_doc = function (opts) {
	function _map() {
	...
	$.each(frappe.boot.service_level_agreement_doctypes, function (_i, d) {
		frappe.ui.form.on(d, {
			onload: function (frm) {
				...
				frappe.call({
					method: "erpnepal.support.doctype.service_level_agreement.service_level_agreement.get_service_level_agreement_filters",
					args: {
						doctype: frm.doc.doctype,
...
$.extend(erpnepal.stock.utils, {
	set_item_details_using_barcode(frm, child_row, callback) {
		const barcode_scanner = new erpnepal.utils.BarcodeScanner({ frm: frm });
