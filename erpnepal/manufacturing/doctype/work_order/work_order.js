...
frappe.ui.form.on("Work Order", {
	setup: function (frm) {
		...
		frm.set_query("bom_no", function () {
			if (frm.doc.production_item) {
				return {
					query: "erpnepal.controllers.queries.bom",
		...
		frm.set_query("operation", "required_items", function () {
			return {
				query: "erpnepal.manufacturing.doctype.work_order.work_order.get_bom_operations",
				filters: {
		...
		frm.set_query("sales_order", function () {
			if (frm.doc.production_item) {
				return {
					query: "erpnepal.manufacturing.doctype.work_order.work_order.query_sales_order",
					filters: {
	...
	onload: function (frm) {
		...
		if (frm.doc.__islocal) {
			...
			erpnepal.work_order.set_default_warehouse(frm);
		}

		if (frm.doc.docstatus == 0 && frm.doc.bom_no) {
			frappe.call({
				method: "erpnepal.manufacturing.doctype.work_order.work_order.check_if_scrap_warehouse_mandatory",
				args: {
	...
	refresh: function (frm) {
		...
		erpnepal.work_order.set_custom_buttons(frm);
		frm.set_intro("");
	...
	create_stock_return_entry: function (frm) {
		frappe.call({
			method: "erpnepal.manufacturing.doctype.work_order.work_order.make_stock_return_entry",
			args: {
	...
	make_job_card: function (frm) {
		...
		const dialog = frappe.prompt(
			...
			function () {
				...
				frappe.call({
					method: "erpnepal.manufacturing.doctype.work_order.work_order.make_job_card",
					freeze: true,
					args: {
						work_order: frm.doc.name,
	...
	make_disassembly_order(frm) {
		erpnepal.work_order
			...
			.then((data) => {
				...
				return frappe.xcall("erpnepal.manufacturing.doctype.work_order.work_order.make_stock_entry", {
	...
	production_item: function (frm) {
		if (frm.doc.production_item) {
			frappe.call({
				method: "erpnepal.manufacturing.doctype.work_order.work_order.get_item_details",
				args: {
				...
				callback: function (r) {
					if (r.message) {
						...
						erpnepal.in_production_item_onchange = true;
						...
						erpnepal.in_production_item_onchange = false;
					}
				},
			});
		}
	...
	project: function (frm) {
		if (!erpnepal.in_production_item_onchange && !frm.doc.bom_no) {
	...
	additional_operating_cost: function (frm) {
		erpnepal.work_order.calculate_cost(frm.doc);
		erpnepal.work_order.calculate_total_cost(frm);
	},
...
frappe.ui.form.on("Work Order Item", {
	...
	source_warehouse: function (frm, cdt, cdn) {
		...
		if (!row.item_code) {
			...
			frappe.call({
				method: "erpnepal.stock.utils.get_latest_stock_qty",
				args: {
	...
	item_code: function (frm, cdt, cdn) {
		...
		if (row.item_code) {
			frappe.call({
				method: "erpnepal.stock.doctype.item.item.get_item_details",
				args: {
					item_code: row.item_code,
					company: frm.doc.company,
				},
...
erpnepal.work_order = {
			...
			frm.add_custom_button(
				...
				function () {
					frappe.confirm(__("Once the Work Order is Closed. It can't be resumed."), () => {
						erpnepal.work_order.change_work_order_status(frm, "Closed");
					});
				},
				__("Status")
			);
		...
		if (doc.docstatus === 1 && !["Closed", "Completed"].includes(doc.status)) {
			if (doc.status != "Stopped" && doc.status != "Completed") {
				frm.add_custom_button(
					__("Stop"),
					function () {
						erpnepal.work_order.change_work_order_status(frm, "Stopped");
					},
					__("Status")
				);
				...
				frm.add_custom_button(
					__("Re-open"),
					function () {
						erpnepal.work_order.change_work_order_status(frm, "Resumed");
					},
					__("Status")
				);
			}

			erpnepal.work_order.setup_stock_reservation(frm);
				...
				if (show_start_btn) {
					...
					if (pending_to_transfer && frm.doc.status != "Stopped") {
						...
						frm.add_custom_button(__("Create Pick List"), function () {
							erpnepal.work_order.create_pick_list(frm);
						});

						var start_btn = frm.add_custom_button(__("Start"), function () {
							erpnepal.work_order.make_se(frm, "Material Transfer for Manufacture");
						});
							...
							frm.add_custom_button(
								...
								function () {
									...
									erpnepal.work_order
										...
										.then((data) => {
											return frappe.xcall(
												"erpnepal.manufacturing.doctype.work_order.work_order.make_stock_entry",
												{
													work_order_id: frm.doc.name,
													purpose: purpose,
													qty: data.qty,
						...
						if (counter > 0) {
							frm.add_custom_button(
								...
								function () {
									...
									erpnepal.work_order.make_consumption_se(
										frm,
					...
					if (flt(doc.material_transferred_for_manufacturing) > 0) {
						if (flt(doc.produced_qty) < flt(doc.material_transferred_for_manufacturing)) {
							...
							let finish_btn = frm.add_custom_button(__("Finish"), function () {
								erpnepal.work_order.make_se(frm, "Manufacture");
							});
	...
	setup_stock_reservation(frm) {
		if (frm.doc.docstatus === 1 && frm.doc.reserve_stock) {
			if (
				...
				frm.add_custom_button(
					__("Reserve"),
					() => erpnepal.stock_reservation.make_entries(frm, "required_items"),
			...
			if (frm.events.has_reserved_stock(frm)) {
				if (frm.doc.skip_transfer || frm.doc.material_transferred_for_manufacturing < frm.doc.qty) {
					frm.add_custom_button(
						__("Unreserve"),
						() => erpnepal.stock_reservation.unreserve_stock(frm),
						__("Stock Reservation")
					);
				}
				...
				frm.add_custom_button(
					__("Reserved Stock"),
					() => erpnepal.stock_reservation.show_reserved_stock(frm, "required_items"),
					__("Stock Reservation")
				);
			}
		}
	...
	set_default_warehouse: function (frm) {
		if (!(frm.doc.wip_warehouse || frm.doc.fg_warehouse)) {
			frappe.call({
				method: "erpnepal.manufacturing.doctype.work_order.work_order.get_default_warehouse",
				args: {
					company: frm.doc.company,
				},
				callback: function (r) {
	...
	make_se: function (frm, purpose, qty, is_additional_transfer_entry) {
		if (qty) {
			frappe
				.xcall("erpnepal.manufacturing.doctype.work_order.work_order.make_stock_entry", {
					work_order_id: frm.doc.name,
					purpose: purpose,
					qty: qty,
	...
	create_pick_list: function (frm, purpose = "Material Transfer for Manufacture") {
		this.show_prompt_for_qty_input(frm, purpose)
			.then((data) => {
				return frappe.xcall("erpnepal.manufacturing.doctype.work_order.work_order.create_pick_list", {
	...
	make_consumption_se: function (frm, backflush_raw_materials_based_on) {
		...
		frappe.call({
			method: "erpnepal.manufacturing.doctype.work_order.work_order.make_stock_entry",
			args: {
				work_order_id: frm.doc.name,
	...
	change_work_order_status: function (frm, status) {
		...
		frappe.call({
			method: `erpnepal.manufacturing.doctype.work_order.work_order.${method_name}`,
			freeze: true,
